#!/usr/bin/env python3
"""Design V10 short, clean, formula-defined microfluidic shapes.

V10 responds to a stricter geometry requirement than V9:

* no free-form or visually arbitrary centerlines;
* short channels are preferred;
* every family has a named mathematical form;
* inlet focusing and electrode-gap parameters are explicit design variables.
...
OUT_DIR = ROOT / "results" / "design_v10_short_clean_shapes"
...
def centerline(sample: dict, n: int = 1200) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    if sample["inlet_kind"] in {"straight_prefocus", "major_axis_prefocus", "dual_side_electrode_entry", "rounded_oval_entry", "outer_low_curvature_entry", "tangential"}:
        ...
        out.append(
            {
                ...
                "v10_score": float(np.mean([r["v10_score"] for r in items])),
            }
        )
    out.sort(key=lambda r: (r["v10_score"], r["mean_target_correct"]), reverse=True)
    return out
...
def plot_gallery(validation: list[dict], samples: list[dict]) -> None:
    ...
    for ax in axes[len(validation[:6]) :]:
        ax.axis("off")
    fig.suptitle("V10 short clean formula finalists; color shows DEP activation", fontsize=12)
    fig.tight_layout()
    fig.savefig(OUT_DIR / "v10_clean_shape_gallery.png", dpi=260)
    plt.close(fig)
...
def plot_metrics(rows: list[dict], validation: list[dict], controls: list[dict]) -> None:
    ...
    ax.set_title("V10 short clean shape screening")
    ax.legend(fontsize=6, ncol=2)
    fig.tight_layout()
    fig.savefig(OUT_DIR / "v10_accuracy_vs_length.png", dpi=260)
    plt.close(fig)
    ...
    ax.set_ylabel("Target correct")
    ax.set_title("V10 held-out validation and controls")
    ax.legend()
    fig.tight_layout()
    fig.savefig(OUT_DIR / "v10_validation_controls.png", dpi=260)
    plt.close(fig)
    ...
    ax.set_title("V10 geometry contribution check")
    fig.tight_layout()
    fig.savefig(OUT_DIR / "v10_topology_gain_vs_length.png", dpi=260)
    plt.close(fig)
...
def main() -> int:
    ...
    rng = np.random.default_rng(10110)
    ...
    for i, sample in enumerate(samples):
        rows.append(run_row(sample, field_stats, i, 91_000 + i, "clean_shape_initial", 55, 220))

    model = ExtraTreesRegressor(n_estimators=420, random_state=101, min_samples_leaf=2, n_jobs=-1)
    model.fit(np.array([feature(samples[int(r["sample_id"])]) for r in rows]), np.array([r["v10_score"] for r in rows]))
    importances = [
        {"feature": k, "importance": float(v)}
        for k, v in sorted(zip(FEATURES, model.feature_importances_), key=lambda kv: kv[1], reverse=True)
    ]
    write_csv(importances, OUT_DIR / "v10_ml_feature_importance.csv")
    ...
    for sample, score in zip(virtual, pred):
        sample["predicted_v10_score"] = float(score)

    selected: list[dict] = []
    for sample in sorted(virtual, key=lambda s: s["predicted_v10_score"], reverse=True):
        if len(selected) >= 30:
            break
    ...
    for j, sample in enumerate(selected):
        ...
        rows.append(run_row(sample, field_stats, sample_id, 92_000 + j, "ml_selected_clean_shape", 90, 300))

    screen_summary = aggregate(rows)

    validation_rows: list[dict] = []
    control_rows: list[dict] = []
    for item in screen_summary[:9]:
        sample = samples[int(item["sample_id"])]
        for seed in [93_001, 93_019, 93_041, 93_073]:
            validation_rows.append(run_row(sample, field_stats, int(item["sample_id"]), seed, "heldout_clean_shape", 150, 360))
        for control_name, kwargs in [
            ...
            scores, losses = [], []
            for seed in [94_003, 94_021, 94_039]:
                particles, summary, _ = simulate(sample, field_stats, seed, 120, 300, **kwargs)
    ...
    write_csv(rows, OUT_DIR / "v10_screening_rows.csv")
    write_csv(screen_summary, OUT_DIR / "v10_screening_summary.csv")
    write_csv(validation_rows, OUT_DIR / "v10_validation_replicates.csv")
    write_csv(control_rows, OUT_DIR / "v10_control_rows.csv")
    write_csv(validation, OUT_DIR / "v10_validation_summary.csv")
    ...
    readme.extend(
        [
            ...
            f"- passes V10 gate: `{best['passes_v10_gate']}`",
            "",
            ...
            "The selected V10 geometry should be rebuilt with an electrode-resolved field solve before a final device claim.",
            "",
