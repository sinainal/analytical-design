...
"""Render Design V2 ParaView trajectory files with a fixed camera."""
...
PARAVIEW_DIR = ROOT / "results" / "openfoam_design_v2" / "paraview"
...
def configure_view(width: int, height: int):
    ...
    view.CameraParallelScale = 0.0023
    return view
...
def render_still(paraview_dir: Path, out_dir: Path, width: int, height: int) -> None:
    ...
    Render(view)
    SaveScreenshot(
        str(out_dir / "design_v2_best_validation_trajectories_still.png"),
        view,
...
def main() -> int:
    ...
    print(f"Rendered Design V2 ParaView frames to {args.out_dir}")
    return 0
